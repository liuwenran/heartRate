...
def test_face(net, data):
    ...
    testlen = len(data['label'])
    firstPath = ['' for x  in range(testlen)]
    ...
    for i in xrange(signalsum):
        ...
        layerLabel = net.layers[0].now_firstPath['label']
        blobLabel = net.blobs['label'].data[0]
        if  abs(layerLabel - blobLabel) > 0.001:
            print 'waaaaaaaaaaaaaarning : gt label not matched'
            print 'layerLabel is ' + str(layerLabel) + ' blobLabel is ' + str(blobLabel)
        firstPath[i] = net.layers[0].now_firstPath['firstPath']

        classoutRate = np.floor(classout * 200)
        gt_labelRate = np.floor(gt_label * 200)
        if classoutRate > (gt_labelRate - 10) and classoutRate < (gt_labelRate + 10):
            print 'this is 1'
            accuracy += 1
        else:
            print 'this is 0'

    return accuracy,result,originLabel, firstPath


if __name__ == '__main__':
    datafile = '/net/liuwenran/datasets/HCI/experiment/ex3_cnn_face/finalExData_shuffled/test_float_seperate.h5'
    test_prototxt = '../proto/ex3_test_loadconv.prototxt'
    rootPath = '/net/liuwenran/datasets/HCI/experiment/videoFrame/RoughFace/'
    pretrained_model = '../data/output_face_test/ex3_HCI_loadppretrain_later3w_iter_30000.caffemodel'
    ...
    accuracy,result,originLabel, firstPath = test_face(net, test_data)
    ...
    np.save('../data/output/HCI_result_later3w_forseperate.npy', result)
    np.save('../data/output/HCI_label_later3w_forseperate.npy', originLabel)
    np.save('../data/output/HCI_firstPath_later3w_forseperate.npy', firstPath)
