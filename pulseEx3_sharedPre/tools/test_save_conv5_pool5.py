...
def test_save_conv5_pool5(net, data):
    signalsum = data['facePath'].shape[0]
    originLabel = []
    result = []
    savelength = 10
    conv5 = [ [] for x in range(savelength)]
    pool5 = [ [] for x in range(savelength)]

    net.layers[0].set_data(data)
    for i in xrange(savelength):
        ti = Timer()
        ti.tic()
        blob_out = net.forward()
        ...
        # conv5 = conv5.append(net.blobs['conv5plus'].data)
        # pool5 = pool5.append(net.blobs['pool5'].data)
        conv5[i] = net.blobs['conv5plus'].data
        pool5[i] = net.blobs['pool5'].data
        # classoutRate = np.floor(classout * 100)
        # gt_labelRate = np.floor(gt_label * 100)
        # if classoutRate > (gt_labelRate - 5) and classoutRate < (gt_labelRate + 5):
        #     print 'this is 1'
    ...
    return result,originLabel,conv5,pool5
...
if __name__ == '__main__':
    ...
    result,originLabel,conv5,pool5 = test_save_conv5_pool5(net, test_data)
    ...
    # np.save('../data/output/test_result_loadpretrain_6w.npy', result)
    # np.save('../data/output/test_label_loadpretrain_6w.npy', originLabel)
    sio.savemat('../data/output/conv5_pool5_loadpretrain_6w.mat', {'result':result, 
                'originLabel':originLabel, 'conv5':conv5, 'pool5':pool5} )
