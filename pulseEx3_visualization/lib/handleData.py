...
def imlist_to_blob(imlist):
    ...
    for i,imName in enumerate(imlist):
        ...
        for channel in range(3):
            # blob[i,channel,:,:] = im[:,:,channel]
            channelMean = sum(im[:,:,channel].ravel())/im[:,:,channel].size
            immean[i, channel] = channelMean
            blob[i,channel,:,:] = im[:,:,channel] - channelMean
...
def get_blob(firstPath, rootPath, label, imNum):
    ...
    blob = {}
    temp, channelMean = imlist_to_blob(imlist)
    blob['data'] = temp
    blob['label'] = np.zeros(1)
    blob['label'][0] = label.astype('float')
    return blob, channelMean
