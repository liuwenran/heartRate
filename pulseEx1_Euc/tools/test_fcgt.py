...
def test_fcgt(net, data):
    ...
    for i in xrange(signalsum):
        ...
        # class_output = blob_out['fc3']
        # class_sorted = np.argsort(-class_output)
        # if class_sorted[0,0] > (label[0,0,0,0] - 5) and class_sorted[0,0] < (label[0,0,0,0] + 5):
        #     accuracy += 1
        class_output = blob_out['sigScore']
        classout = class_output[0,0] * 100
        labelin = label[0,0,0,0] * 100
        if classout > (labelin - 5) and classout < (labelin + 5):
            accuracy += 1
...
if __name__ == '__main__':
 ...
 #   testfile = '/net/liuwenran/caffe_learn/data/lwr_test_img_mat.mat'
    test_prototxt = '../proto/ex1_euc_test.prototxt'
    output_dir = '../data/output/'
    pretrained_model = '../data/output/fcgt_1024_iter_10000.caffemodel'
    gpu_id = 1
